...
Validate include
- Edit it server and tool level, all permutations, serialization, application (right tools actually in context on new session)

Display of tools in heirarchy
- Server (xx of yy tools) or (all tools)
  - List of tools

Similar heirachy in modal
- How much server and tool info?  Tool name/desc, probably no params
- Add/remove on server or tool
  - If add server, add tools based on include (if default or always, add)
  - On remove server, remove all tools
