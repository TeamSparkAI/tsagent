...
export class DockerProvider implements Provider {
  ...
  static getInfo(): ProviderInfo {
    return {
      name: "Docker",
      description: "Docker Model Runner - OpenAI-compatible API running in Docker (e.g., LocalAI, vLLM, etc.)",
      configValues: [
        {
          caption: "Base URL",
          key: "BASE_URL",
  ...
  static async validateConfiguration(agent: Agent, config: Record<string, string>): Promise<{ isValid: boolean, error?: string }> {
    ...
    if (!baseUrl) {
      return { isValid: false, error: 'BASE_URL is missing in the configuration. Please add it to your config.json file.' };
    }
    try {
      ...
      return { isValid: false, error: 'Failed to validate Docker configuration: ' + (error instanceof Error && error.message ? ': ' + error.message : '') };
    }
  }
    ...
    try {
      ...
      for (const message of messages) {
        if ('modelReply' in message) {
            ...
            if (turn.results) {
              for (const result of turn.results) {
                if (result.type === 'toolCall') {
                  // Push the tool call result - ensure content is always a string
                  turnMessages.push({
                    role: 'tool',
          ...
          // Handle regular messages - ensure content is always a string
          turnMessages.push({
        ...
        for (const toolCallApproval of lastChatMessage.toolCallApprovals) {
          ...
          if (toolCallApproval.decision === 'allow-session' || toolCallApproval.decision === 'allow-once') {
            ...
            if (toolResult.content[0]?.type === 'text') {
              ...
              // Add the tool call (executed) result to the context history - ensure content is always a string
      ...
      this.logger.info('Docker Provider generated response with messages:', JSON.stringify(turnMessages, null, 2));
      ...
      while (turnCount < state.maxChatTurns) {
        ...
        this.logger.info(`Docker Provider processing turn ${turnCount} with messages:`, JSON.stringify(turnMessages, null, 2));
        ...
        if (!response) {
          throw new Error('No response from Docker provider');
        }

        // this.logger.info('Docker response', JSON.stringify(response, null, 2));
        ...
        if (response.tool_calls && response.tool_calls.length > 0) {
          ...
          //
          // Extract only valid OpenAI API fields - exclude non-standard fields like reasoning_content
          //
          // Note: With Docker Model Runner, we've seen some models return a content value of null when making a tool call, and when
          //       you feed that message back into the API later it blows up because of the null content value.  So we will always 
          //       make sure it has at least an empty string content value.
          turnMessages.push({
            role: 'assistant',
            ...
            if (toolCall.type === 'function') {
              ...
              if (await session.isToolApprovalRequired(toolServerName, toolToolName)) {
                ...
                if (toolResult.content[0]?.type === 'text') {
                  ...
                  // Record the tool result in the message context - ensure content is always a string
                  turnMessages.push({
                    role: 'tool',
      ...
      this.logger.info('Docker response generated successfully');
      return modelReply;
    } catch (error: unknown) {
      this.logger.error('Docker API error:', error instanceof Error ? error.message : 'Unknown error');
      modelReply.turns.push({
        error: `Error: Failed to generate response from Docker provider - ${error instanceof Error ? error.message : 'Unknown error'}`
      });
      return modelReply;            
    }
  }
